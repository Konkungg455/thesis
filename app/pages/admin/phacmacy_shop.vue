...
const handleActionConfirm = async () => {
  ...
  try {
    ...
    if (currentAction === 'delete') {
      const res = await $fetch(apiUrl(`delete-store.php?id=${id}`), { credentials: 'include' })
      showResultPopup('success', 'ลบข้อมูลแล้ว', res?.message || 'ลบออกจากหน้าปกติแล้ว')
    } else {
      const res = await $fetch(apiUrl(`restore-deleted.php?type=store&id=${id}`), { credentials: 'include' })
