...
const serverRemaining = ref(CONSULT_DURATION_SECONDS);
...
const fetchMessages = async () => {
    ...
    try {
        ...
        if (data) {
            ...
            const latestMedicineRequest = [...data]
                .reverse()
...
const selectPatient = (id) => {
    ...
    // รีเฟรชอาการให้ตรงรอบที่เลือก (ไม่ล้างทิ้งก่อน → ไม่วูบหาย)
...
const initPharmacyFromRoute = (newId) => {
    if (!import.meta.client) return;
...
onMounted(async () => {
    ...
    initPharmacyFromRoute(route.query.id);
                    ...
                    <div class="chat-container chat-container--pharma">
                        ...
                        <div class="chat-messages" ref="chatScroll" @scroll.passive="onChatScroll">
                            ...
                            <template v-for="msg in chatMessagesWithDivider" :key="msg.__divider ? msg.__key : msg.message_id">
                                ...
                                <!-- ปุ่มแก้ไข/ลบ — แสดงเฉพาะข้อความของตัวเอง -->
                                <div v-if="msg.sender_role === 'pharma' && editingMessageId !== msg.message_id && canModifyMessage(msg)"
                                     class="msg-actions">
                                    <button v-if="msg.message_text" type="button" class="msg-act-btn"
                                            @click="startEditMessage(msg)" title="แก้ไขข้อความ">
